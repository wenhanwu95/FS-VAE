...
unseen_zs = np.load(f'{wdir}/{le}/{tm}/MSF_{str(ss)}_r_unseen_zs.npy')    
seen_zs = np.load(f'{wdir}/{le}/{tm}/MSF_{str(ss)}_r_seen_zs.npy')        
unseen_train = np.load(f'{dataset}/ztest_out.npy')                         
seen_train = np.load(f'{dataset}/val_out.npy')                             
...
for f in [ss]:
    ...
    for t in t_iter:
        ...
        val_unseen_inds = np.random.choice(np.arange(feat_unseen_train.shape[0]), feat_unseen_train.shape[0] // 5, replace=False)  
        val_seen_inds = np.random.choice(np.arange(feat_seen_train.shape[0]), feat_seen_train.shape[0] // 5, replace=False)       
        ...
        best = 0
        bestT = 0
        th_iter = [i for i in range(25, 75, 1)] if th_set == 0 else [th_set] 
        for th in th_iter:
            y = prob[:, 0] > th/100
